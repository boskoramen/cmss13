	...
	if(xeno_save)
		...
		if(xeno_save["caste_stats_list"])
			...
			for(var/save_caste in save_caste_list)
				...
				if(save_caste["actions_used"])
					var/save_actions_used = save_caste["actions_used"]
					var/list/new_actions_used_list = new()
					for(var/save_action in save_actions_used)
						var/datum/entity/statistic/new_action_used = new()
						new_action_used.name = save_action["name"]
						new_action_used.value = save_action["value"]
						new_actions_used_list["[new_action_used.name]"] = new_action_used
					new_caste.actions_used = new_actions_used_list
